"""
BİLGE AI — UI v3
Deep purple aurora · Constellation background
"""
...
SYSTEM_NAME = "BİLGE AI"
MODEL_BADGE = "AKILLI ASİSTAN · WINDOWS"
TAGLINE = "Kişisel Yapay Zeka Asistanı"

# ── Renk paleti (mor-altın aurora) ───────────────────────────────────────────
C_BG      = "#050812"
C_PRI     = "#c9a0ff"
C_ORG     = "#f5b942"
C_ORG2    = "#ffd978"
...
class JarvisUI:
    def __init__(self):
        ...
        self._bilge_state   = "INITIALISING"
        ...
        self._wave_bilge = [random.randint(4, 26) for _ in range(18)]
        ...
        try:
            bg_path = BASE_DIR / "background.png.jpg"
            if bg_path.exists():
                self._background_image = Image.open(bg_path)
                self._background_image = self._background_image.resize((self.W, self.H), Image.LANCZOS)
    ...
    def _enter_fullscreen(self):
        ...
        # Windows'ta tam ekran için zoomed state kullanıyoruz
    ...
    def _resize_surface(self, width: int, height: int):
        ...
        # Background image'i yeniden boyutlandır
    ...
    def _build_social_bar(self):
        ...
        name_lbl.bind("<Button-1>", _open("https://www.instagram.com/mit.eyup"))
        ...
        if self._icon_ig:
            ...
            ig_lbl.bind("<Button-1>", _open("https://www.instagram.com/mit.eyup"))
    ...
    def _build_autostart_plist(self) -> str:
        ...
        out_log    = BASE_DIR / "bilge.log"
        err_log    = BASE_DIR / "bilge_error.log"
        home       = Path.home()
...
"""
    ...
    def _toggle_autostart(self):
        try:
            ...
            if self._is_autostart_installed():
                ...
                self.write_log("SYS: Otomatik başlatma açıldı. Windows açılışında BİLGE AI başlar.")
    ...
    def _create_desktop_shortcut(self):
        def _run():
            try:
                ...
                self.write_log("SYS: Masaüstü kısayolu oluşturuldu → BİLGE AI.lnk")
    ...
    def _sync_sound_state(self):
        ...
        if enabled and self._bilge_state == "THINKING":
    ...
    def _toggle_pause(self):
        ...
        if self.paused:
            ...
            self.write_log("SYS: BİLGE AI duraklatıldı.")
        else:
            self.set_state("THINKING")
            self.write_log("SYS: BİLGE AI devam ediyor...")
        self._sync_sound_state()
    ...
    def _shutdown(self):
        self.sound.stop_all()
        self.write_log("SYS: BİLGE AI kapatılıyor...")
    ...
    def _on_input_submit(self, event=None):
        ...
        if not text:
            return
        if self.paused:
            self.write_log("SYS: BİLGE AI duraklatılmış durumda. Devam etmek için pause'u kapat.")
            return
        self._input_var.set("")
    ...
    def set_state(self, state: str):
        previous = getattr(self, "_bilge_state", "")
        self._bilge_state = state
    ...
    def _state_color(self, state: str | None = None) -> str:
        effective = state or self._bilge_state
        if effective == "PAUSED":
            return C_MID
    ...
    def _start_typing(self):
        if not self.typing_queue:
            self.is_typing = False
            if self._bilge_state == "ERROR" and time.time() < self._error_hold_until:
                return
        ...
        elif tl.startswith("bilge:") or tl.startswith("bilge ai:") or tl.startswith("ai:"): tag = "ai"
    ...
    def _animate(self):
        ...
        if self.speaking and t % 3 == 0:
            self._wave_bilge = [random.randint(6, 30) for _ in range(18)]
        ...
        if now - self.last_t > (0.12 if self.speaking else 0.50):
            ...
            elif self._bilge_state in ("THINKING", "INITIALISING"):
    ...
    def _orb_rgb(self):
        state = "PAUSED" if self.paused else self._bilge_state
    ...
    def _draw_right_panel(self, c):
        ...
        if self.paused:
            ...
            sc, st = self._state_color(self._bilge_state), self._bilge_state
    ...
    def _draw(self):
        ...
        # ── Arka plan — resim veya mor aurora + yıldız alanı ────────────────────
        if self._background_photo:
            c.create_image(0, 0, image=self._background_photo, anchor="nw")
        else:
            c.create_rectangle(0, 0, W, H, fill=C_BG, outline="")

            # Merkez aurora parlaması
            cx, cy = W * 0.52, H * 0.46
            for i, (rad, alpha) in enumerate([(420, 18), (300, 28), (180, 38), (90, 50)]):
                c.create_oval(
                    cx - rad, cy - rad, cx + rad, cy + rad,
                    fill=self._ac(60, 120, 200, alpha), outline="",
                )
            for i, (rad, alpha) in enumerate([(260, 12), (140, 22)]):
                c.create_oval(
                    W * 0.18 - rad, H * 0.72 - rad, W * 0.18 + rad, H * 0.72 + rad,
                    fill=self._ac(40, 80, 160, alpha), outline="",
                )

            # Yatay aurora bantları
            aurora_y = (t * 0.35) % (H + 120) - 60
            for band in range(3):
                ly = (aurora_y + band * 90) % H
                c.create_line(0, ly, W, ly + 48, fill=self._ac(50, 90, 170, 14 + band * 4), width=1)

            # Yıldız partikülleri (parıltılı)
            R, G, B = self._orb_rgb()
            for p in self.particles:
                twinkle = 0.55 + 0.45 * math.sin(t * 0.04 + p.get('tw', 0))
                alpha = int(p['a'] * twinkle)
                if self.speaking:
                    col = self._ac(245, 185, 80, alpha)
                else:
                    col = self._ac(160, 200, 255, alpha)
                r = p['r']
                c.create_oval(p['x']-r, p['y']-r, p['x']+r, p['y']+r, fill=col, outline="")
                if r > 1.2 and twinkle > 0.85:
                    c.create_line(p['x']-r*2, p['y'], p['x']+r*2, p['y'],
                                  fill=self._ac(255, 240, 200, alpha // 3), width=1)
                    c.create_line(p['x'], p['y']-r*2, p['x'], p['y']+r*2,
                                  fill=self._ac(255, 240, 200, alpha // 3), width=1)
        ...
        state_label = "PAUSED" if self.paused else self._bilge_state
        ...
        # ── HEADER ───────────────────────────────────────────────────────────
        c.create_rectangle(0, 0, W, HDR_H, fill="#060310", outline="")
        # Alt çizgi — mor parlak
        c.create_line(0, HDR_H, W, HDR_H, fill=C_MID, width=1)
        for i in range(3):
            a = 60 - i * 18
            c.create_line(0, HDR_H-1-i, W, HDR_H-1-i,
                          fill=self._ac(160, 100, 255, a), width=1)
        ...
        indicator_state = "PAUSED" if self.paused else self._bilge_state
    ...
    def _save_api_key(self):
        ...
        if was_ready:
            ...
            self.set_state("LISTENING")
            self.write_log("SYS: BİLGE AI hazır. Dinliyorum...")
